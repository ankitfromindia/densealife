 ...
 */
class Plugin_Friend extends Plugin
{
    ...
    public function feeds()
    {
        ...
        $this->load->library('comments/comments') ;
        return $this->comments->feeds($limit) ;
    }
    
    public function is_friend()
    {
        $uid = $this->attribute('uid');
        $this->load->library('friends/friends');
        return $this->friend->is_friend($uid);
    }
    
    // Plugin will be used to fetch the all the events that a logged in user's friends following.
