...
//			$folder->name_indent		= repeater('&raquo; ', $depth) . $folder->name;
	...
	public function get_by_path($path)
	{
        ...
        public function get_albums($event_id)
	{
		if ($event_id)
		{
			$this->album_tree($event_id);
		}
		elseif (empty($this->_folder))
		{
			$this->album_tree();
        ...
        public function album_tree($event_id, $parent_id = 0, $depth = 0, &$arr = array())
	{
		$arr = $arr ? $arr : array();
		...
		$folders = $this
			->order_by('file_folders.name')
			->get_many_by(array('parent_id' => $parent_id,'event_id' => $event_id));
...
//			$folder->name_indent		= repeater('&raquo; ', $depth) . $folder->name;
        ...
        public function get_albums_by_user_id($user_id)
        {
            $albums = $this
			->order_by('file_folders.name')
			->get_many_by(array('user_id' => $user_id));
            return $albums;
