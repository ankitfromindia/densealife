...
class Files
{
     ...
     *
     * */
    public static function folder_contents($parent = 0, $event_id = null, $by_user = false, $user_id = null)
    {
        ...
        if($user_id!=''){
            ci()->file_folders_m->where('user_id', $user_id);
        }
        ...
        if ( $folders ) {
            foreach ( $folders as &$folder ) {
                ...
                $content = ci()->file_m
                                ...
                                ->order_by('sort','desc')
                                ->get_all() ;
                $content_first = current($content);
               
                if($content_first->path!=''){
                    $folder->folder_image = $content_first->filename;
                }
                
                foreach ( $content as &$item ) {
                    $item->keywords_hash  = $item->keywords ;
                    $item->keywords       = ci()->keywords->get_string($item->keywords) ;
                    $item->formatted_date = format_date($item->date_added) ;
                }
     ...
     * @return array
     */
    public static function folder_tree($event_id = null)
    {
        $folders      = array() ;
        $folder_array = array() ;

        ci()->db->select('id, parent_id, slug, name')
                ->where('hidden', 0);
    ...
    public static function upload($folder_id, $name = false, $field = 'userfile', $width = false, $height = false, $ratio = false, $allowed_types = false, $alt = NULL, $replace_file = false)
    {
        ...
        if ( $folder || empty($folder_id) ) {
            ...
            if ( ci()->upload->do_upload($field) ) {
                ...
                if ( !empty($folder_id) && $folder->location !== 'local' ) {
     ...
     *
     * */
    public static function create_album($parent = 0, $name = 'Untitled Folder', $event = 0, $by_user= false, $privacy = 'public', $location = 'local', $remote_container = '', $hidden = 0)
    {
        $i             = '' ;
        $original_slug = self::create_slug($name) ;
        $original_name = $name ;
        ...
        $slug = $original_slug ;
        while ( ci()->file_folders_m->count_by(array('slug' =>$slug, 'user_id' => ci()->current_user->id)) ) {
            $i++ ;
            $slug = $original_slug . '-' . $i ;
            $name = $original_name . '-' . $i ;
        }
    ...
    public static function change_access_level($file_id, $access){
        return ci()->file_m->update_access_level($file_id, $access);
    }
}
