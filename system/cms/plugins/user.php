...
//                exit;
	...
	public function __call($name, $data)
	{
        ...
        public function profile_thumb(){
            $user_email  = $this->attribute('email');
            $size = $this->attribute('size', 60);
            return gravatar($user_email, $size);
        }
        ...
        public function profile_pic()
        {
            ...
            if(!empty($pic_info)){
                $x = current($pic_info);
                return '<a href="/user/'.$username.'">'.img(array( 'src' => 'files/thumb/' . $x['id'] . '/'.$width.'/'.$height.'/'.$mode)).'</a>';
            }else{
                return img(array('src' => '/assets/images/no-profile-pic.png', 'width' => $width, 'height' =>$height));
            }
        }
        
        public function profile_pic_180()
        {
            $user_id  = $this->attribute('user_id');
            $pic_info = $this->get_user_var('profile_pic', $user_id);
            $username = $this->get_user_var('username', $user_id);
            if(!empty($pic_info)){
                $x = current($pic_info);
                return '<a href="/user/'.$username.'">'.img(array( 'src' => 'files/thumb/' . $x['id'] . '/180/180/fit')).'</a>';
            }else{
        ...
        {
//            $user_id  = $this->attribute('user_id', $user_id);
//            echo count($this->friends($user_id));
            return false; 
            return count($this->friends($user_id)) ? true : false;
        }
